...
class Standard < Solr::Request::Select
  ...
  VALID_PARAMS = [:query, :sort, :default_field, :operator, :start, :rows,
    :filter_queries, :field_list, :debug_query, :explain_other, :facets, :highlighting, :query_function]
  ...
  def initialize(params)
    ...
    @params[:query_function] = params[:query_function]
  end
  ...
  def to_hash
    ...
    # use either sort or query_function
    if sort
      hash[:q] = "#{@params[:query]};#{sort}"
    elsif @params[:query_function]
      hash[:q] = "#{@params[:query]} _val_:#{@params[:query_function]}"
    else
      hash[:q] = @params[:query]
    end
