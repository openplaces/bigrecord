...
namespace :solr do
  ...
  task :start do
    begin
      ...
      Dir.chdir(SOLR_PATH) do
        system "nohup java -Dsolr.data.dir=solr/data/#{ENV['RAILS_ENV']} -Djetty.port=#{SOLR_PORT} -jar start.jar >> #{SOLR_PATH}/tmp/#{ENV['RAILS_ENV']}.log 2>&1 < /dev/null & \n echo $! > #{SOLR_PATH}/tmp/#{ENV['RAILS_ENV']}.pid"
        sleep(5)
  ...
  task :stop do
    fork do
      file_path = "#{SOLR_PATH}/tmp/#{ENV['RAILS_ENV']}.pid"
