...
1. Install the Bigrecord Driver gem and its dependencies. Please see the Bigrecord Driver documentation for more detailed instructions. (http://github.com/openplaces/bigrecord/tree/master/bigrecord-driver/)

2. Add the following line into the Rails::Initializer.run do |config| block:

  config.gem "bigrecord", :source => "http://gems.github.com"

Or, run the following command to install it as a plugin:

  script/plugin install git://github.com/openplaces/bigrecord.git

3. Create or edit the config/bigrecord.yml[.sample] file in your Rails root to the information corresponding to your database install.
